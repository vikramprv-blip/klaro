    ...
    for (const client of clients) {
      for (let month = 1; month <= 12; month++) {
        ...
        await prisma.$executeRaw`
          ...
          values
          (${client.id}, ${client.name}, 'gst', ${title}, 'Prepare and file monthly GST return.', ${period}, ${gstDueDate(year, month)}::date, 'pending', 'high', 'auto')
      ...
      for (let quarter = 1; quarter <= 4; quarter++) {
        ...
        await prisma.$executeRaw`
          ...
          (client_id, client_name, task_type, title, description, period, due_date, status, priority, source)
          values
          (${client.id}, ${client.name}, 'tds', ${title}, 'Review challans and prepare quarterly TDS filing.', ${period}, ${tdsDueDate(year, quarter)}::date, 'pending', 'high', 'auto')
          on conflict (client_id, task_type, period) do nothing
        `;
        created++;
      }
      ...
      await prisma.$executeRaw`
        ...
        (client_id, client_name, task_type, title, description, period, due_date, status, priority, source)
        values
        (${client.id}, ${client.name}, 'itr', ${itrTitle}, 'Collect documents and prepare income tax return.', ${itrPeriod}, ${itrDueDate(year + 1)}::date, 'pending', 'medium', 'auto')
        on conflict (client_id, task_type, period) do nothing
      `;
      created++;
    }
