...
export async function GET(request: Request) {
  try {
    ...
    return NextResponse.json({ message: 'Error generating notification delivery effectiveness summary: ' + (error instanceof Error ? error.message : String(error)) }, { status: 500 });
  }
}
