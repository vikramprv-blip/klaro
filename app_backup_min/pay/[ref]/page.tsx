...
async function getLink(ref: string) {
  try {
...
export default async function PayPage({ params }: { params: { ref: string } }) {
  const link = await getLink(params.ref) as any
  if (!link) notFound()
      ...
      <div style={{ width:"100%", maxWidth:420, background:"#111827", border:"1px solid rgba(255,255,255,0.08)", borderRadius:20, overflow:"hidden" }}>
        <div style={{ background:"rgba(99,102,241,0.1)", borderBottom:"1px solid rgba(255,255,255,0.07)", padding:"20px 24px", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
          <div style={{ display:"flex", alignItems:"center", gap:10 }}>
            ...
            <div>
              <p style={{ fontSize:13, fontWeight:600, color:"#F1F5F9" }}>{name}</p>
        ...
        <div style={{ padding:28 }}>
          <h1 style={{ fontSize:18, fontWeight:800, marginBottom:6, color:"#F1F5F9" }}>{link.title}</h1>
          ...
          <div style={{ background:"#1A2235", borderRadius:12, padding:18, marginBottom:20 }}>
            ...
            <div style={{ display:"flex", justifyContent:"space-between", fontSize:17, fontWeight:800, borderTop:"1px solid rgba(255,255,255,0.07)", paddingTop:10, marginTop:4, color:"#F1F5F9" }}>
              <span>Total due</span>
              <span style={{ color:"#6366F1" }}>{sym}{Number(link.total_amount).toLocaleString()}</span>
            </div>
          </div>
          {paid && (
            <div style={{ background:"rgba(16,185,129,0.1)", border:"1px solid rgba(16,185,129,0.2)", borderRadius:10, padding:"14px 18px", textAlign:"center" }}>
              <p style={{ fontSize:15, color:"#10B981", fontWeight:700 }}>Payment completed</p>
            </div>
          )}
          {!paid && !expired && (
            <>
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
                {link.gateways?.includes("upi") && merchant?.upi_id && (
                  <div style={{ background:"#1A2235", border:"1px solid rgba(99,102,241,0.2)", borderRadius:12, padding:16 }}>
                    <p style={{ fontSize:14, fontWeight:600, marginBottom:8, color:"#F1F5F9" }}>UPI — Instant</p>
                    <div style={{ background:"#0A0F1E", borderRadius:8, padding:"10px 14px" }}>
                      <span style={{ fontFamily:"monospace", fontSize:13, color:"#F1F5F9" }}>{merchant.upi_id}</span>
                    </div>
              ...
              <p style={{ fontSize:11, color:"#94A3B8", textAlign:"center", marginTop:16 }}>
                Secure payment · Expires {new Date(link.expires_at).toLocaleDateString("en-IN")}
              </p>
            </>
          )}
          {expired && !paid && (
