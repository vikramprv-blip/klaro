...
const NAV = [
  { section:"VARO", items:[
    { href:"/varo/app",               icon:"🤖", label:"Dashboard" },
  ...
  { section:"ACCOUNT", items:[
    { href:"/varo/app/settings",      icon:"⚙️", label:"Settings" },
  ]},
]
  ...
  return (
    <aside style={{ position:"fixed", top:0, left:0, bottom:0, width:220, background:"#0D1117", borderRight:"1px solid rgba(255,255,255,0.06)", padding:"16px 10px", display:"flex", flexDirection:"column", zIndex:40 }}>
      <Link href="/" style={{ display:"flex", alignItems:"center", gap:10, marginBottom:20, paddingLeft:8, textDecoration:"none" }}>
        <div style={{ width:28, height:28, borderRadius:7, background:"#8B5CF6", display:"flex", alignItems:"center", justifyContent:"center", fontWeight:900, fontSize:13, color:"#fff" }}>🤖</div>
        <div>
          <span style={{ fontSize:14, fontWeight:800, color:"#F1F5F9" }}>Varo</span>
          <span style={{ fontSize:10, color:"#8B5CF6", fontFamily:"monospace", marginLeft:6 }}>by klaro</span>
        </div>
      </Link>
      ...
      {NAV.map(section => (
        <div key={section.section} style={{ marginBottom:16 }}>
          ...
          {section.items.map(n => (
            <Link key={n.href} href={n.href} style={{ display:"flex", alignItems:"center", gap:8, padding:"7px 10px", borderRadius:7, fontSize:13, color:isActive(n.href)?"#F1F5F9":"#6B7280", background:isActive(n.href)?"rgba(139,92,246,0.15)":"transparent", textDecoration:"none", fontWeight:isActive(n.href)?500:400, borderLeft:isActive(n.href)?"2px solid #8B5CF6":"2px solid transparent", marginBottom:1 }}>
      ...
      <div style={{ flex:1 }} />
      <div style={{ padding:"10px 8px", background:"rgba(139,92,246,0.08)", border:"1px solid rgba(139,92,246,0.15)", borderRadius:9, marginBottom:6 }}>
        <p style={{ fontSize:9, fontFamily:"monospace", color:"#8B5CF6", marginBottom:3 }}>{merchant.plan.toUpperCase()}</p>
