...
export default function WorkboardPage({
  ...
  function handleDragEnd(event: DragEndEvent) {
    ...
    const allowed = ALLOWED_TRANSITIONS[draggedItem.status] || [];
    if (!allowed.includes(targetStatus)) {
      showToast("Invalid move", `Cannot move from ${draggedItem.status} to ${targetStatus}`);
      return;
    }
    ...
    setItems(optimistic);
    persistStatusChange(draggedItem.id, targetStatus, previousItems);
  }
